...
def get_tickets_z_fs_tickets(con_ticket, per_limit : int = None, offset_limit : int  = None, date_limiter : bool = True, selected_tickets = None):
    ...
    default_where = [
        "WHERE (department_id IS NULL OR department_id IN (23000035894,23000038553,23000203546,23000038482,23000038541,23000209665,23000038546,23000157750,23000038547,23000038548,23000038550,23000070194))", # Clientes incluidos Adaptive Security,AFP Capital,AFP Habitat,Aguas Andinas,BDO Chile,BNP Paribas Cardif,COPEC,Consorcio,Emaresa S.A.,Evertec Inc.,SBPay,Seguros Continental
        "AND (plataforma IS NULL OR plataforma NOT IN ('Beyond Trust'))",                   # Excluido de Monitoreo
        "AND (category IS NULL OR category IN ('Reportes', 'Monitoreo'))",                  # SLA Monitoreo
        "AND (sub_category IS NULL OR sub_category IN ('SIEM', 'Informe de Servicio'))",    # SLA Monitoreo
        "AND (deleted != 1)",                                                               # Descarte de borrados
        "AND (ticket_fusionado IS NULL OR ticket_fusionado = 0)",                           # Corrector
        "AND (group_id IS NULL OR group_id NOT IN (23000086944))",                          # Corrector
    ]
    default_where = " ".join(default_where)
...
def detector_insert_update_delete(fs_tickets, sla_tickets, list_customers, ignore_date = False):
...
def build_ticket_data(single_ticket, list_customers):
    ...
    if single_ticket['category'] == 'Reportes' and ('Mensual', 'Semanal', 'Diario', 'On-Demand'):
        timer_crear     = None 
        timer_cerrar    = comparador_fechas(single_ticket['closed_at'], single_ticket['created_at'])  # Tiempo de diferencia de crear para un ticket
        timer_notificar = comparador_fechas(single_ticket['closed_at'], single_ticket['created_at'])  # Tiempo de diferencia de cerrar para un ticket
        ...
        timer_crear     = date_diferencia(single_ticket['created_at'], single_ticket['fechahora_ofensa']) # Tiempo de diferencia de crear para un ticket
        timer_cerrar    = date_diferencia(single_ticket['closed_at'], single_ticket['fechahora_ofensa'])  # Tiempo de diferencia de cerrar para un ticket
        timer_notificar = date_diferencia(single_ticket['closed_at'], single_ticket['fechahora_ofensa'])
...
sla_table        = 'sla_monitoreo'

# Controlador por ticket especifico
selected_tickets = None  # Default None [51885, 52063, 51844, 52532, 51845, 52348] 
...
if enable_delete == True:
    ...
    # del delete_offset_limit, delete_per_limit, delete_loop_breaker, sla_tickets, fs_tickets
...
if selected_tickets != None and selected_tickets != []: # Ejecucion "Forzada"
    ...
    fs_tickets  = detector_insert_update_delete(fs_tickets, sla_tickets, list_customers, True)
    ...
    while loop_breaker == False:
        ...
        if fs_tickets != None and fs_tickets != []:
            ...
            sla_tickets = get_tickets_sla_selected(con_monthly, sla_table, fs_tickets)
            fs_tickets  = detector_insert_update_delete(fs_tickets, sla_tickets, list_customers, ignore_date)

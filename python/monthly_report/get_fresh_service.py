...
def fresh_service_get(var, tn = None): 
...
def list_corrector(ticket):
    for keys in ticket:
        ...
        if isinstance(ticket[keys], list):
            if ticket[keys] == []:
                ticket[keys] = None
            elif isinstance(ticket[keys][0], dict):
                pass
            else:
...
def dictionary_field_corrector(tickets):
    ...
    def for_asset(tickets, container_var):
        ...
        for ticket in tickets[container_var]:
            ...
            ticket.update(ticket.pop('stats'))
...
def limit_warning(response, responsable):
    try:
        ...
        if usage_percent >= 0.45: # limite recomendado 45%
...
# tickets = fresh_service_get("ticket_specific")

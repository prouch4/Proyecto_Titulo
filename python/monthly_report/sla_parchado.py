...
def get_tickets_z_fs_tickets(con_ticket, per_limit : int = None, offset_limit : int  = None, date_limiter : bool = True, selected_tickets = None):
    ...
    # Filtros fijos
    default_where = [
        "WHERE (department_id IN (23000038446))",
        "AND (category IS NULL OR category IN ('Reportes', 'Parchado'))",
        "AND (producto IS NULL OR producto IN ('Big Fix'))",
        "AND (ticket_fusionado is NULL OR ticket_fusionado = 0)",
        "AND (deleted != 1)",
        "AND (group_id NOT IN (23000086944))",
    ]
...
def build_ticket_data(single_ticket, list_customers):
    ...
    if single_ticket['item_category'] in ('Informe de parches disponibles', 'Notificación de parches disponibles', 'Programacion de Parchado','Ejecución de Parchado','Informe de parchado','Reporte Semanal','Reporte Mensual','Reporte On-Demand'):
...
print(f"\n{init_datetime} - Iniciando script sla_parchado.py")
...
print(f"{stop_datetime} - Finalizado script sla_parchado.py")
...
def get_tickets_z_fs_tickets(con_ticket, per_limit : int = None, offset_limit : int  = None, date_limiter : bool = True, selected_tickets = None):
    ...
    # Filtros fijos
    default_where = [
        "WHERE (department_id IS NULL OR department_id NOT IN (23000173121,23000038546,23000070194,23000038543,23000038600,23000174227,23000038552,23000078999,23000038554,23000047133))", # SURA, Consorcio, Seguros Continental, BTG Pactual Chile, Clínica Alemana, COCHILCO, SMU, Universidad Adolfo Ibañez, Universidad de las Américas, VTR Test
        "AND (category IS NULL OR category IN ('Reportes', 'Administración'))",
        "AND (ticket_fusionado IS NULL OR ticket_fusionado = 0)",
        "AND (group_id IS NULL OR group_id NOT IN (23000086944))",
        "AND (deleted != 1)",
        "AND (item_category IS NULL OR item_category NOT IN ('Revisión de Respaldos'))",
    ]
...
sla_table        = 'sla_administracion'

# Controlador por ticket especifico
selected_tickets = None  # Default None
...
print(f"\n{init_datetime} - Iniciando script sla_administracion.py")
...
if selected_tickets != None and selected_tickets != []: # Ejecucion "Forzada"
    ...
    # IF para los DELETE en DB
    if fs_tickets['delete'] != None:
        print(f"Tickets para Delete: {len(fs_tickets['delete'])}")
        for fs_ticket in fs_tickets['delete']:
            temp_var = construir_delete_and_data(fs_ticket, sla_table)
            list_queries.append(temp_var['query'])
            list_datas.append(temp_var['data'])
...
print(f"{stop_datetime} - Finalizado script sla_administracion.py")